...
import {
  ...
  CountryInfo,
  CountryInfoDto,
  CountryInfoFlagResponseDto,
  CountryInfoPopulationResponseDto,
  FlagDto,
  PopulationDto,
} from 'src/public/model/country';
...
export class CountryService implements ICountryService {
  ...
  async getCountryInfo(countryCode: string): Promise<CountryInfo> {
    try {
      ...
      const borderCountriesResponse =
        await axios.get<CountryInfoDto>(nagerDateUri);
      const borderCountries: Country[] =
        borderCountriesResponse.data.borders.map((border: CountryInfoDto) => ({
      ...
      const countryPopulationResponse = (
        await axios.get<CountryInfoPopulationResponseDto>(countryPopulationUri)
      ).data.data;
      const countryPopulation = countryPopulationResponse.find(
        (country: PopulationDto) => country.country === countryName,
      )?.populationCounts;

      const countryFlagUri = `${process.env.COUNTRY_INFO_API_URL}/countries/flag/images`;
      const countryFlagResponse = (
        await axios.get<CountryInfoFlagResponseDto>(countryFlagUri)
      ).data.data;
      const countryFlag = countryFlagResponse.find(
        (flag: FlagDto) => flag.name === countryName,
      )?.flag;
